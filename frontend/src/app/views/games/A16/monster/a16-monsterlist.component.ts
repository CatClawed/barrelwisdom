...
@Component({
  ...
  export class A16MonsterlistComponent implements OnInit {
    ...
    ngOnInit(): void {
      ...
      this.getMonsters();
      this.seoService.createCanonicalURL(`shallie/monsters/${this.language}`);
      this.titleService.setTitle(`Monsters - Atelier Shallie - Barrel Wisdom`);
      this.metaService.updateTag({ name: `robots`, content: `index, archive` },`name="robots"`);
      this.metaService.updateTag({ name: `description`, content: `The list of monsters in Atelier Shallie.` }, `name="description"`);
      this.metaService.updateTag({ property: `og:title`, content: `Monsters` }, `property="og:title"`);
      this.metaService.updateTag({ property: `og:description`, content: `The list of monsters in Atelier Shallie.` },`property="og:description"`);
    ...
    openModal(template: TemplateRef<any>, slugname: string) {
      ...
      this.location.go('shallie/monsters/' + slugname + "/" + this.language);
      this.modalRef = this.modalService.show(template);
      this.modalRef.onHide.subscribe((reason: string | any) => {
          this.location.go('shallie/monsters/' + this.language);
        })
    }

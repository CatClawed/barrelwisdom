...
@Component({
  ...
  export class A16ItemlistComponent implements OnInit {
    ...
    ngOnInit(): void {
      ...
      this.getItems();
      this.getCategories();
      this.seoService.createCanonicalURL(`shallie/items/${this.language}`);
      this.titleService.setTitle(`Items - Atelier Shallie - Barrel Wisdom`);
      this.metaService.updateTag({ name: `robots`, content: `index, archive` },`name="robots"`);
      this.metaService.updateTag({ name: `description`, content: `The list of items in Atelier Shallie.` }, `name="description"`);
      this.metaService.updateTag({ property: `og:title`, content: `Items` }, `property="og:title"`);
      this.metaService.updateTag({ property: `og:description`, content: `The list of items in Atelier Shallie.` },`property="og:description"`);
    ...
    openModal(template: TemplateRef<any>, slugname: string) {
      this.item = slugname;
      this.location.go('shallie/items/' + slugname + "/" + this.language);
      this.modalRef = this.modalService.show(template);
      this.modalRef.onHide.subscribe((reason: string | any) => {
          this.location.go('shallie/items/' + this.language);
        })
    }
